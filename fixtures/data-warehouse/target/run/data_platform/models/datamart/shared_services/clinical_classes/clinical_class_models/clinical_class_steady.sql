...
create or replace transient table dw_dev.dev_jkizer.clinical_class_steady
    copy grants
...
with base_appointments as (
    select
        ...
        appointment_type_category as program,
        'Steady Strides/Pasos Firmes' as class_name,
    ...
    where appointment_type_category = 'Steady Strides'
...
select
    ...
    case when fpt.tag_value = 'Steady Strides' then tag_value else null end as tag_value,
    date(fpt.creation_datetime) as tag_date,
    case
        when apts_attended_in_cohort >= 6 then 'Graduated' else 'Not Graduated'
...
left join dw_dev.dev_jkizer.fct_patient_tag fpt
    ...
    and fpt.tag_value = 'Steady Strides'
