...
create or replace transient table dw_dev.dev_jkizer.clinical_class_mh_toc
    copy grants
...
with base_appointments as (
    select
        ...
        'MH-T' as program,
        case when appointment_description like '%2.0%' then 'MH: Transitions of Care 2.0'
            else 'MH: Transitions of Care'
                end as class_name,
    ...
    where (appointment_type_category like 'MH: Transitions of Care%')
...
select
    ...
    case when fpt.tag_value = 'MH T' then tag_value else null end as tag_value,
