...
create or replace transient table dw_dev.dev_jkizer.clinical_class_pt_one_one
    copy grants
...
with base_appointments as (
    select
        ...
        case when appointment_type_category like '1:1%' then 'PT-1:1' end as program,
        '1:1' as class_name,
    ...
    where (lower(appointment_type_category) like '1:1%')
    and
        (appointment_provider_category = 'Physical Therapy')
...
select
    ...
    case when fpt.tag_value = 'PT- 1:1' then tag_value end as tag_value,
