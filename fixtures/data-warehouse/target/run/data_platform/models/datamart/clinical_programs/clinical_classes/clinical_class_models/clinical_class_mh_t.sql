...
create or replace transient table dw_dev.dev_jkizer.clinical_class_mh_t
    copy grants
...
with base_appointments as (
    select
        ...
        'MH-T' as program,
        case
            when appointment_type_category = 'Viviendo con el Duelo' then 'MH-T: Viviendo con el Duelo'
            when appointment_type = 'MH: Palabras Sanadoras' then 'MH-T: Palabras Sanadoras'
                else 'MH-T' end as class_name,
    ...
    where (appointment_type like '%MH T:%' or appointment_type like 'MH: Palabras Sanadoras' or appointment_type_category like 'Viviendo con el Duelo')
...
select
    ...
    case when fpt.tag_value in ('MH-T', 'MH-Palabras Sanadoras') then tag_value else null end as tag_value,
    date(fpt.creation_datetime) as tag_date,
    null as graduation_status
from final c
join graduation g
...
left join dw_dev.dev_jkizer.fct_patient_tag fpt
    ...
    and fpt.tag_value in ('MH-T', 'MH-Palabras Sanadoras')
