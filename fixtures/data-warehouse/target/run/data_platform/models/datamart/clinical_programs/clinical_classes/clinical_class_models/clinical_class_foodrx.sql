...
create or replace transient table dw_dev.dev_jkizer.clinical_class_foodrx
    copy grants
...
with base_appointments as (
    select
        ...
        case when lower(appointment_type_category) like 'food rx%' then 'Food RX' else null end as program,
    ...
    where lower(appointment_type_category) like 'food rx%'
...
select
    ...
    case when fpt.tag_value = 'Food RX' then fpt.tag_value end as tag_value,
...
left join dw_dev.dev_jkizer.fct_patient_tag fpt
    ...
    and lower(fpt.tag_value) = 'food rx'
