...
create or replace transient table dw_dev.dev_jkizer.int_patient_measure_count_uat
    copy grants
...
from dw_dev.dev_jkizer.int_patient_summary_uat p
left join (
	select suvida_id, sum(count) as "count"
	from (
		select suvida_id, count(distinct quality_measure_skey) as "count"
		from dw_dev.dev_jkizer.int_patient_quality_measure_uat
		where
		...
		from dw_dev.dev_jkizer.int_patient_med_adherence_uat
		where
...
left join (
	...
	from dw_dev.dev_jkizer.int_patient_attestation_opportunity_uat
