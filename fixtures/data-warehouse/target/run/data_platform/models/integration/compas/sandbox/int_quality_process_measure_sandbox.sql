...
create or replace transient table dw_dev.dev_jkizer.int_quality_process_measure_sandbox
    copy grants
...
right join dw_dev.dev_jkizer.int_patient_summary_sandbox pt
    on md5(cast(coalesce(cast(qpm.suvida_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) = pt.suvida_id
where
