...
create or replace transient table dw_dev.dev_jkizer.int_patient_attestation_opportunity
    copy grants
    
    
    as (with coder_attestation_opportunities as (
    select
...
left join dw_dev.dev_jkizer.int_patient_summary ips
    on iao.suvida_id = ips.suvida_id
...
where
    ips.suvida_id is not null
qualify row_number() over (partition by ips.suvida_id, iao.attestation_opportunity_skey, iao.attestation_opportunity_version_skey order by ecg.date desc) = 1
    )
;
