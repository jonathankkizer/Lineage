
  
    

create or replace transient table dw_dev.dev_jkizer.dim_elation_current_custom_block
    copy grants
    
    
    as (-- Table grain is one row per question and answer pairing per custom_block/template 
-- There can be multiple answers per question, this table captures all possibilities 

--- Current State of a Custom Block. The different versions of a custom block are in custom_block_snapshot

with base as (
  select 
    custom_block_id,
    label, 
    copied_from_block_id,
    created_at,
    created_by_user_id,
    edited_at, 
    hdb_last_sync,
    current_state_snapshot[0]:attrs:dataSchema:properties as properties,
    current_state_snapshot[0]:attrs:data as data
  from dw_dev.dev_jkizer_staging.stg_elation_vn2_custom_block
  where deleted_at is null
),

-- flatten properties
properties_flat as (
  select
    base.custom_block_id,
    base.label, 
    base.copied_from_block_id,
    base.created_at,
    base.created_by_user_id,
    base.edited_at, 
    base.hdb_last_sync,
    property.key::string as question_field_name,
    property.value:title::string as question_title,
    property.value:type::string as question_type,
    property.value:format::string as question_format,
    property.value:oneOf as single_select_options,
    property.value:items:oneOf as multi_select_options,
    base.data[property.key::string] as answer_raw
  from base,
  lateral flatten(input => base.properties) property
),

-- for string type with oneof (single select questions) - show all options
single_select_questions as (
  select
    pf.custom_block_id,
    pf.label, 
    pf.copied_from_block_id,
    pf.created_at,
    pf.created_by_user_id,
    pf.edited_at, 
    pf.hdb_last_sync,
    pf.question_field_name,
    pf.question_title,
    pf.question_type,
    pf.question_format,
    opt.value:const::string as answer_value,
    opt.value:title::string as answer_title
  from properties_flat pf
  cross join lateral flatten(input => pf.single_select_options, outer => true) opt
  where pf.question_type = 'string' 
    and pf.single_select_options is not null
),

-- for array type with items.oneof (multi-select questions) - show all options
multi_select_questions as (
  select
    pf.custom_block_id,
    pf.label, 
    pf.copied_from_block_id,
    pf.created_at,
    pf.created_by_user_id,
    pf.edited_at, 
    pf.hdb_last_sync,
    pf.question_field_name,
    pf.question_title,
    pf.question_type,
    pf.question_format,
    opt.value:const::string as answer_value,
    opt.value:title::string as answer_title
  from properties_flat pf
  cross join lateral flatten(input => pf.multi_select_options, outer => true) opt
  where pf.question_type = 'array'
    and pf.multi_select_options is not null
),

-- for simple fields (no options - just string/date inputs)
simple_fields as (
  select
    custom_block_id,
    label, 
    copied_from_block_id,
    created_at,
    created_by_user_id,
    edited_at, 
    hdb_last_sync,
    question_field_name,
    question_title,
    question_type,
    question_format,
    null as answer_value,
    null as answer_title
  from properties_flat
  where question_type not in ('array')
    and single_select_options is null
),

-- combine all results
combined as (
select 
    custom_block_id,
    label, 
    copied_from_block_id,
    created_at,
    created_by_user_id,
    edited_at, 
    hdb_last_sync,
    question_field_name,
    question_title,
    question_type,
    question_format,
    answer_value,
    answer_title
from single_select_questions

union all

select 
    custom_block_id,
    label, 
    copied_from_block_id,
    created_at,
    created_by_user_id,
    edited_at, 
    hdb_last_sync,
    question_field_name,
    question_title,
    question_type,
    question_format,
    answer_value,
    answer_title
from multi_select_questions

union all

select 
    custom_block_id,
    label, 
    copied_from_block_id,
    created_at,
    created_by_user_id,
    edited_at, 
    hdb_last_sync,
    question_field_name,
    question_title,
    question_type,
    question_format,
    answer_value,
    answer_title,
from simple_fields

order by custom_block_id, question_field_name, answer_value
) 

select 
md5(cast(coalesce(cast(custom_block_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(question_field_name as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(answer_value as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as custom_block_skey,
*
from combined
    )
;


  