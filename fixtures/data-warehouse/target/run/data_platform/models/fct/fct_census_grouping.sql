...
create or replace transient table dw_dev.dev_jkizer.fct_census_grouping
    copy grants
    
    
    as (/* for general approach, look into "gaps and groupings problem" */
...
with data as ( -- get relevant data and start/stops; if discharge_date isn't available, use admit_date and assume 1-day admission
	select 
		suvida_id, 
		coalesce(admit_date, discharge_date) as admit_date, 
		coalesce(discharge_date, admit_date) as admit_end_date
	from dw_dev.dev_jkizer.intmdt_census_event
	where (admit_date is not null or discharge_date is not null)
	and suvida_id is not null 
	and level_of_care in ('inpatient', 'emergency', 'observation')
	...
	select
		*,
		case when datediff(day, previous_admit_end_date, admit_date) <= 1 then 0 else 1 end as grouping_start_ind,
		sum(case when datediff(day, previous_admit_end_date, admit_date) <= 1 then 0 else 1 end) over (partition by suvida_id order by rn) as census_grouping_id
	from groupings
	...
	select
		...
		case when 
			datediff(day, previous_admit_end_date, admit_date) <= 1 
			then 0 
			else 1 
...
select 
	* exclude (census_grouping_id),
	md5(cast(coalesce(cast(suvida_id as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(census_grouping_id as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as census_grouping_id,
