  ...
  create or replace   view dw_dev.dev_jkizer_staging.stg_wellcare_census
  ...
  as (
    select
   SUBSCRIBER_ID as source_member_id,
   date(AD_DATE, 'DD-Mon-YY') as admit_date,
   date(DIS_DATE, 'DD-Mon-YY') as discharge_date,
   PCP as pcp,
   'inpatient' as level_of_care,
   coalesce(sfn.facility_name, cd.FACILITY) as facility,
   DIAG_CODE as dx_code,
   DIAG_TEXT as dx_text,
...
coalesce(
 ...
 as report_date,
   ...
   'sftp' as source_type,
from airbyte_source_prod.wellcare_tx.census_sftp cd
left join dw_dev.dev_jkizer_staging.stg_sharepoint_suvida_facility_name sfn
    on cd.FACILITY = sfn.facility_code
    and sfn.code_source = 'WellCare/Centene'

union all

select
    replace(MEMBER_ID, '-01', '') as source_member_id,
    to_date(to_varchar(split(nullif(admit_date, 'N/A'), ' ')[0]), 'MM/DD/YYYY') as admit_date,
    to_date(to_varchar(split(nullif(discharge_date, 'N/A'), ' ')[0]), 'MM/DD/YYYY') as discharge_date,
    PCP_NAME as pcp,
    'inpatient' as level_of_care,
    coalesce(sfn.facility_name, cd.FACILITY) as facility,
    null as dx_code,
    diagnosis as dx_text,
    split(_ab_source_file_url, '/')[array_size(split(_ab_source_file_url, '/'))-1]::varchar as src_file_name,
...
from airbyte_source_prod.wellcare_tx.census_portal_inpatient cd
left join dw_dev.dev_jkizer_staging.stg_sharepoint_suvida_facility_name sfn
    on cd.FACILITY = sfn.facility_code
    and sfn.code_source = 'WellCare/Centene'
