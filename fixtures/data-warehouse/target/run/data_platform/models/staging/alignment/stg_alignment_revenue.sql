  ...
  create or replace   view dw_dev.dev_jkizer_staging.stg_alignment_revenue
  ...
  as (
    ...
    select * from airbyte_source_prod.alignment.revenue
...
renamed as (
    ...
    select
        ...
        data['Member']::varchar as member_name,
        data['Program']::varchar as program,
        try_to_date(data['PCP Eff.']::varchar, 'MM/DD/YYYY') as pcp_effective_date,
        try_to_date(data['Pay Period']::varchar, 'MM/DD/YYYY') as pay_period_date,
        try_to_double(replace(replace(data['Premium']::varchar, '$', ''), ',', '')) as premium,
        try_to_double(replace(replace(data['Capitation']::varchar, '$', ''), ',', '')) as capitation,
        try_to_double(replace(data['Rate']::varchar, '%', '')) as rate_pct,
        nullif(data['Adjustment']::varchar, '') as adjustment,
        nullif(data['Excl. Pgm']::varchar, '') as excluded_program,
        split_part(_ab_source_file_url, '/', 5) as src_file_name,
...
final as (
    select
        md5(cast(coalesce(cast(member_file_skey as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(pay_period_date as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(premium as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(adjustment as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(excluded_program as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as id,
        *
    from renamed
)
