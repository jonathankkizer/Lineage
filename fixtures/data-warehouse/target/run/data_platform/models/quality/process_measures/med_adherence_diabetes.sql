...
create or replace transient table dw_dev.dev_jkizer_quality.med_adherence_diabetes
    copy grants
  ...
  select
    ...
    pat.quality_measure,
	  concat(
      'Next Refill Due: ', coalesce(pat.next_refill_due::string, null),
      '. Perc Days Covered: ', coalesce(pat.perc_days_covered::string, null),
      '. Real Time GDR: ', coalesce(pat.real_time_gdr::string, null),
      '. Stage: ', coalesce(stage_med_adh_outreach, null)
    ) as evidence_desc,
    ...
    where pat.quality_measure = 'Med Adherence - Diabetes'
