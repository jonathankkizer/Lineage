...
with unpivot_cte as (
	select
		...
		trim(f.value::string) as icd_10_code,
		hcpcs_code,
		split(hcpcs_modifier_codes, ', ')[0]::varchar as hcpcs_modifier_1,
		claim_type,
		claim_start_date as diagnosis_date,
		member_id,
	from dw_dev.dev_jkizer_staging.stg_wellmed_medical_claim_retro as t,
	lateral flatten(input => split(t.dx_codes, ', ')) as f
	where claim_id_rn = 1
), med_claim_data as (
	select
		data_source,
		...
		hcpcs_code,
		claim_start_date,
	from dw_dev.dev_jkizer_staging.stg_wellmed_medical_claim_retro
), risk_adj_status as (
	select 
		data_source,
		claim_id,
		...
		max(
			case when lower(claim_type) = 'professional' and cpt.hcpcs_cpt_code is not null then true
			else false
			end
		) as is_professional_risk_adjustable,
		max(
			case when lower(claim_type) = 'institutional' and substring(bill_type_code, 1, 2) in ('11','41') then true
			else false
			end
		) as is_inpatient_risk_adjustable,
		max(
			case when lower(claim_type) = 'institutional' and substring(bill_type_code, 1, 2) in ('12','13','43','71','73','76','77','85')
			then true
			else false
			end
...
left join (select claims_diagnosis_skey from dw_dev.dev_jkizer.fct_claims_diagnosis_retro) as unique_keys on unique_keys.claims_diagnosis_skey = final.claims_diagnosis_skey
