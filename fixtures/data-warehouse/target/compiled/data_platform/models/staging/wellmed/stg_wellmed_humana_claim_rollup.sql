with data as (
	select
		left(data:MBR_PID::varchar, len(data:MBR_PID::varchar) - 2) || '-01' as member_id,
		try_to_date(nullif(nullif(data:SRV_FRM_DT::varchar, ''), 'NULL'), 'YYYYMMDD') as claim_start_date,
		try_to_date(nullif(nullif(data:SRV_TO_DT::varchar, ''), 'NULL'), 'YYYYMMDD') as claim_end_date,
		case when lower(data:claim_type::varchar) = 'inst' then 'institutional'
			when lower(data:claim_type::varchar) = 'prof' then 'professional'
		end as claim_type,
		try_to_double(nullif(nullif(data:FUND_EXP::varchar, ''), 'NULL')) as paid_amount,
...
coalesce(
 ...
 as report_date,
	from airbyte_source_prod.wellmed.claims_humana
)
select 
	*,
	dense_rank() over (partition by claim_start_date order by date(report_date) desc) as _rn,
from data