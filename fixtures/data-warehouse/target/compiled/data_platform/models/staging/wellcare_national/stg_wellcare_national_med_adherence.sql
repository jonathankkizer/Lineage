-- Wellcare/Centene national med adherence feed (covers AZ + TX after the wellcare_tx
-- airbyte source was deprecated). Two report types are unioned: full report and single fill.
-- Date fields arrive as Excel serial integers; the coalesce(dateadd, try_to_date) pattern
-- handles both serial integers and ISO strings if the encoding ever flips.

with full_report as (
    select
        coalesce(data:Member_ID::varchar, data:Subscriber_ID::varchar) as member_id,
        svh_qm.quality_measure,
        svh_qm.quality_measure_type,
        svh_qm.measure_weight,
        data:Measure_Key::varchar as payer_quality_measure,
        iff(svh_qm.quality_measure is null, false, true) as payer_suvida_measure_match,
        concat(data:Compliance_Detail::varchar, ' | ', data:Label_Name::varchar) as measure_detail,
        0 as is_single_fill,
        data:Label_Name::varchar as rx_name,
        null as rx_number,
        data:Compliance_Status::varchar as member_status,
        data:P_DaysCovered::float as perc_days_covered,
        iff(data:Last_Fill_Days_Supply::number >= 90, 0, 1) as ninety_day_opportunity,
        data:Days_to_NonAdh::number as gap_days_remaining,
        concat(data:Last_Fill_Prescriber_First_Name::varchar, ' ', data:Last_Fill_Prescriber_Last_Name::varchar) as prescriber_name,
        data:Prescriber_Phone_Number::varchar as prescriber_phone,
        coalesce(
            dateadd(day, try_to_number(data:Last_Fill_Refill_Date::varchar), '1899-12-30'::date),
            try_to_date(data:Last_Fill_Refill_Date::varchar)
        ) as last_fill_date,
        data:Last_Fill_Days_Supply::number as last_fill_day_supply,
        coalesce(
            dateadd(day, try_to_number(data:Next_Fill_Due_Date::varchar), '1899-12-30'::date),
            try_to_date(data:Next_Fill_Due_Date::varchar)
        ) as next_refill_due,
        null as refills_remaining,
        null as rx_tier,
        coalesce(
            dateadd(day, try_to_number(data:Member_Fill1::varchar), '1899-12-30'::date),
            try_to_date(data:Member_Fill1::varchar)
        ) as first_fill_date,
        data:Number_Of_Fills::number as number_of_fills,
        data:Pharmacy_Name::varchar as pharmacy_name,
        data:Pharmacy_Phone_Number::varchar as pharmacy_phone,
        concat(data:Pharmacy_Address_1::varchar, ' ', data:Pharmacy_City::varchar, ' ', data:Pharmacy_State::varchar) as pharmacy_address,
        data:LOB_State::varchar as lob_state,
...
coalesce(
    ...
    null
        ...
        replace(split(_ab_source_file_url, '/')[array_size(split(_ab_source_file_url, '/'))-2]::varchar, '.parquet', '') as src_file_name,
        'full_report' as report_type
    from airbyte_source_prod.wellcare_national.quality_med_adherence_full wma
    left join dw_dev.dev_jkizer_staging.stg_star_payer_measure_names svh_qm
        on data:Measure_Key::varchar = svh_qm.payer_measure_name
    ...
    select
        ...
        data:MEASURE_KEY::varchar as payer_quality_measure,
        iff(svh_qm.quality_measure is null, false, true) as payer_suvida_measure_match,
        concat(data:LABEL_NAME::varchar, ' | ') as measure_detail,
        1 as is_single_fill,
        data:LABEL_NAME::varchar as rx_name,
        null as rx_number,
        ...
        iff(data:LAST_FILL_DAYS_SUPPLY::number >= 90, 0, 1) as ninety_day_opportunity,
        data:Days_Left::number as gap_days_remaining,
        concat(data:LAST_FILL_PRESCRIBER_FIRST_NAME::varchar, ' ', data:LAST_FILL_PRESCRIBER_LAST_NAME::varchar) as prescriber_name,
        data:PRESCRIBER_PHONE_NUMBER::varchar as prescriber_phone,
        coalesce(
            dateadd(day, try_to_number(data:MEMBER_FILL1::varchar), '1899-12-30'::date),
        ...
        data:LOB_STATE::varchar as lob_state,
        coalesce(
            dateadd(day, try_to_number(data:Report_Ran_Date::varchar), '1899-12-30'::date),
            try_to_date(data:Report_Ran_Date::varchar)
        ) as report_date,
    ...
    from airbyte_source_prod.wellcare_national.quality_med_adherence_single_fill wmsf
    left join dw_dev.dev_jkizer_staging.stg_star_payer_measure_names svh_qm
        on data:MEASURE_KEY::varchar = svh_qm.payer_measure_name
        and svh_qm.payer_name = 'Wellcare/Centene'
)
select * from full_report where last_fill_date is not null
union all
select * from single_fill where last_fill_date is not null